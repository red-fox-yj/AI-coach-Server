...
chartPath = "D:/py/Ai-coach01/data/runningChart/"
...
def segTest(standardList, testList, times):
    ...
    path = fastDtw(testList, standardList)###test在前，tuple中为横坐标,distance:dtw距离，path:dtw路径
    ...
    while ixNext < len(path):
        if path[ix][0] != path[ixNext][0] and path[ix][1] != path[ixNext][1]:
            ...
            while ixNext < len(path) and  path[ix][0] == path[ixNext][0]:
                ixNext += 1
            runningTestList.append(testList[path[ix][0]])
            runningStandardList.append(calMean(standardList, path[ix][1], path[ixNext - 1][1]))
            ix = ixNext
            ixNext += 1
